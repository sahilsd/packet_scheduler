...
void stateRR::dumpStats(float M)
{
    ...
    for(int i=0; i<NUM_SOURCES;i++) {
        snprintf(fname, 64,"%d_tput_rr.dat", i);
        tput = fopen(fname, "a+");
        snprintf(fname, 64, "%d_delay_rr.dat", i);
        delay = fopen(fname, "a+");
        double rate;
...
/*
 * exponential distribution over mean
 */
...
/* Only on departure */
void stateRR::update_stats(int src)
{
    double responseTime;
...
/*
 * Same as FIFO.
 * Does two main jobs: 1) Reset counters 2) Schedule 1st arrival for each source.
 * Only difference is it clears the per flow queues as well.
 */

void stateRR::initSimulation(float M)
{
    r = M/(NUM_SOURCES - 1);
...
/* 
 * Same as FIFO. Take care of scheduling next arrival and departure.
 * For RR policy, next active source is selected for schedule_event()
 */
void stateRR::schedule_event(int type, int pkt, int src)
{
...
/*
 * Push the packet to corresponding source's queue
 */
void stateRR::handle_arrival(int pkt_id, int src_id)
{
#ifdef DEBUG
...
/*
 * Pop packet from corresponding source's queue
 */
void stateRR::handle_departure(int pkt, int src)
{
    update_stats(src);
#ifdef DEBUG
    fprintf(stdout, "[%ld] Dequeue\t %d\ts %d\tp %d\t sz %ld\tserved %ld\n", this->clock,
            this->flows[src].front()->type, this->flows[src].front()->sourceId,
            this->flows[src].front()->packetId, this->flows[src].size(), this->globalStats.totalPackets);
#endif
    this->flows[src].pop();
    this->eventQueue.pop();
#ifdef DEBUG
    ...
    if(this->currentPackets >= 1) {
        ...
        /* this is the main difference of RR policy */
...
void stateRR::simulate(float totalLoad)
{
    r = totalLoad/10;
